		...
		if(!empty($state_info->search))
		{
            ...
            if (is_array($state_info->search)) {
                    ...
                    if (isset($temp_relation[$search_field])) {
                        if (is_array($temp_relation[$search_field])) {
                            ...
                            foreach ($temp_relation[$search_field] as $relation_field) {
                            	...
                            	$searched_field = $this->basic_model->escape_str($relation_field);
                                $escaped_text = $this->basic_model->escape_str($search_text);
                                $temp_where_query_array[] = $searched_field . ' LIKE \'%' . $escaped_text . '%\'';
                            }
                    	...
                    	$searched_field = $this->basic_model->escape_str($search_field);
                        $escaped_text = $this->basic_model->escape_str($search_text);
                        $this->having($searched_field." LIKE '%".$escaped_text."%'");
                    } else {
				...
				if (isset($temp_relation[$state_info->search->field])) {
					...
					$relation_field = $this->basic_model->escape_str($state_info->search->field);
					$escaped_text = $this->basic_model->escape_str($state_info->search->text);
					$this->having($relation_field." LIKE '%".$escaped_text."%'");
				} else {
				...
				foreach($columns as $column)
				{
					if(isset($temp_relation[$column->field_name]))
					{
						if(is_array($temp_relation[$column->field_name]))
						{
							...
							{
								$escaped_field= $this->basic_model->escape_str($search_field);
                                $escaped_text = $this->basic_model->escape_str($search_text);
                                $temp_where_query_array[] = $escaped_field . ' LIKE \'%' . $escaped_text . '%\'';
							}
						}
						else
						{
							$escaped_field= $this->basic_model->escape_str($temp_relation[$column->field_name]);
                            $escaped_text = $this->basic_model->escape_str($search_text);
                            $temp_where_query_array[] = $escaped_field . ' LIKE \'%' . $escaped_text . '%\'';
						}
					}
					...
					elseif (
                    	...
                    	$escaped_field= $this->basic_model->escape_str($column->field_name);
                        $escaped_text = $this->basic_model->escape_str($search_text);
                        $temp_where_query_array[] =  '`' . $basic_table . '`.' . $escaped_field . ' LIKE \'%' . $escaped_text . '%\'';
					}
				}
